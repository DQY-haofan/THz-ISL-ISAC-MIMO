#!/usr/bin/env python3
"""
Supplementary Figure Generation Script - IMPROVED VERSION
IEEE Publication Style - Matching Main Visualization Suite

改进点：
- fig_gamma_breakdown: 移除绝对值科学计数法标签，只保留百分比
...
def generate_figure_gamma_breakdown(config, output_dir='figures'):
    """
    Gamma Breakdown Figure - IMPROVED VERSION
    Shows hardware distortion components with percentage labels ONLY (no scientific notation)

    Changes from original:
    - Removed lines 165-168 (absolute value labels in scientific notation)
    - Only percentage labels remain
    """
    print("\n" + "=" * 80)
    print("FIGURE: Gamma Breakdown (Percentage Only)")
    print("=" * 80)
    ...
    # Add percentage labels ONLY (改进：移除科学计数法标签)
    ...
    if len(small_indices) >= 2:
        ...
        # 改进：inset图也只显示百分比，不显示科学计数法
        for bar, pct in zip(bars_inset, small_pcts):
            height = bar.get_height()
...
def main():
    ...
    print("=" * 80)
    print("SUPPLEMENTARY FIGURES GENERATOR - IMPROVED VERSION")
    print("IEEE Publication Style - Gamma Breakdown with Percentage Only")
    print("=" * 80)
    ...
    if success_count == total_count:
        ...
        print("  - Gamma breakdown: percentage labels ONLY (no scientific notation)")
        print("  - Clean and uncluttered visualization")
        sys.exit(0)
    else:
